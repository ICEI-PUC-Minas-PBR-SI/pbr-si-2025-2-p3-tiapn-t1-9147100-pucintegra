...
import { AvailabilityManager } from './components/landlord/AvailabilityManager';
import { PricingManager } from './components/landlord/PricingManager';
import { ReportsManager } from './components/landlord/ReportsManager';
import { LandlordRatings } from './components/landlord/LandlordRatings';
import { PropertyReviews } from './components/landlord/PropertyReviews';
...
import { TenantReservations, CompletedReservation } from './components/tenant/TenantReservations';
import { Property } from './components/tenant/PropertyCard';
import { RatingModal } from './components/shared/RatingModal';
import { RatingSuccess } from './components/shared/RatingSuccess';
import { TenantToRate } from './components/landlord/LandlordRatings';
...
export default function App() {
  ...
  const [currentFlow, setCurrentFlow] = useState<
    'signup' | 'login' | 'dashboard' | 'property' | 'bookings' | 'availability' | 'pricing' | 'reports' | 'landlord-ratings' | 'landlord-reviews' |
    'tenant-login' | 'tenant-listing' | 'tenant-property-details' | 'tenant-booking' | 'tenant-booking-success' | 'tenant-reservations' | 'tenant-rating-success' |
    'landlord-rating-success'
  >('signup');
  ...
  const [ratingTarget, setRatingTarget] = useState<{
    ...
    name: string;
    data: CompletedReservation | TenantToRate | null;
  }>({ type: 'property', name: '', data: null });
  ...
  const handleOpenRatingModal = (type: 'property' | 'tenant', name: string, data: CompletedReservation | TenantToRate | null) => {
    setRatingTarget({ type, name, data });
  ...
  const handleRatingSubmit = (rating: number, description: string) => {
    console.log('Rating submitted:', { rating, description, target: ratingTarget });
    setSubmittedRating(rating);
    setIsRatingModalOpen(false);
    if (ratingTarget.type === 'property') {
      setCurrentFlow('tenant-rating-success');
    } else if (ratingTarget.type === 'tenant') {
      setCurrentFlow('landlord-rating-success');
    }
  };
          ...
          <div className="w-full">
            ...
            {currentFlow === 'dashboard' && (
              <Dashboard
                ...
                onManageAvailability={() => setCurrentFlow('availability')}
                onManagePricing={() => setCurrentFlow('pricing')}
                onViewReports={() => setCurrentFlow('reports')}
                onViewRatings={() => setCurrentFlow('landlord-ratings')}
                onViewPropertyReviews={() => setCurrentFlow('landlord-reviews')}
            ...
            {currentFlow === 'landlord-ratings' && (
              <LandlordRatings
                onBack={() => setCurrentFlow('dashboard')}
                onRateTenant={(tenant) => handleOpenRatingModal('tenant', tenant.tenantName, tenant)}
              />
            )}

            {currentFlow === 'landlord-reviews' && (
              <PropertyReviews
                onBack={() => setCurrentFlow('dashboard')}
              />
            )}
            ...
            {currentFlow === 'tenant-listing' && (
              <PropertyListing
                ...
                onViewReservations={() => setCurrentFlow('tenant-reservations')}
              />
            )}

            {currentFlow === 'tenant-property-details' && (
              <PropertyDetails
                property={selectedProperty!}
                onBack={() => setCurrentFlow('tenant-listing')}
                onBook={() => setCurrentFlow('tenant-booking')}
              />
            )}
            ...
            {currentFlow === 'tenant-reservations' && (
              <TenantReservations
                userName={userData.nome || 'Usuário'}
                onBack={() => setCurrentFlow('tenant-listing')}
                onRateProperty={(reservation) => handleOpenRatingModal('property', reservation.propertyName, reservation)}
              />
            )}

            {currentFlow === 'tenant-rating-success' && (
              <RatingSuccess
                rating={submittedRating}
                type="property"
                onContinue={() => setCurrentFlow('tenant-reservations')}
              />
            )}

            {currentFlow === 'landlord-rating-success' && (
              <RatingSuccess
                rating={submittedRating}
                type="tenant"
                onContinue={() => setCurrentFlow('landlord-ratings')}
              />
            )}
          </div>
        </main>
